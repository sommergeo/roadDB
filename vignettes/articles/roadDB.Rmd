...
```
```{r echo=FALSE}
cat(paste0(paste(head(road_list_argument_values("technocomplex")$technocomplex, 40), collapse = ", "), ", and many more."))
```
...
In this example, we visualize different technocomplex categories by setting the `col` argument (which determines what variable is used for coloring — here, `mousterian$technocomplex`) and the palette argument (which specifies the color scheme to use).
...
eggshell_misc <- eggshell_misc[grepl("ostrich egg shell", eggshell_misc$miscellaneous_find_material), ] # filter rows for search pattern

# Query from symbolic artifacts
eggshell_symbolic <- road_get_symbolic_artifacts() # download whole dataframe
eggshell_symbolic <- eggshell_symbolic[grepl("ostrich egg shell", eggshell_symbolic$symbolic_artifact_material), ] # filter rows for search pattern
...
We can extend our filter to select this specific archaeological layer and symbolic artifacts.
...
hohle_fels_Vb <- hohle_fels_Vb[grepl("symbolic artifacts", hohle_fels_Vb$category), ]
```
