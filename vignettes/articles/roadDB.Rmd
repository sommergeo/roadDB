...
**Fig 1:** Three levels of detail determine the granularity of a data frame.
...
**Function-specific arguments** are only available for certain road_get_* functions. These arguments refine queries in ways that are unique to the corresponding function. Some of them use *exact* search terms, whereas others use *fuzzy/contains* search.
...
```
As you can see, the map can be customized using the arguments available in  [`tmap::tm_dots()`](https://r-tmap.github.io/tmap/reference/tm_symbols.html). In this example, we visualize different technocomplex categories by setting the `col` argument (which determines what variable is used for coloring — here, `mousterian$technocomplexes`) and the palette argument (which specifies the color scheme to use).

Once you have transferred your dataset to a spatial datatype, there are many interresting packages available for spatial analysis of your data, for example:
...
This is especially true for radiocarbon dates, which are stored uncalibrated in ROAD, and which require you to perform and customize your own age modelling.
...
**Fig 2:** Inside the UNESCO site "Hohle Fels" | [CC BY-SA Thilo Parg](https://commons.wikimedia.org/wiki/File:Hohler_Fels_Halle_Achtal_Schelklingen.jpg) 

Let's have a look at Hohle Fels, a well-known Paleolithic cave site in the Swabian Jura with plenty of dates. First, we'll get an overview of all the available data, and then use [`tidyverse::filter()`](https://dplyr.tidyverse.org/reference/filter.html) to narrow down our results.
...
**Fig 3:** Flute from the Aurignacian layers of Hohle Fels | H. Jensen/Universität Tübingen 
