...
### Install R, RStudio
Please install the current version of the statistical software `R` and the Integrated Development Environment (IDE) `RStudio`, both available from [https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/).

If you would like to install the development version of `roadDB` on MS windows, make sure to install `Rtools` from [https://cran.r-project.org/bin/windows/Rtools/](https://cran.r-project.org/bin/windows/Rtools/).


### Install roadDB
You only need to install the package once on your R setup, so you can skip this step if it's already installed. The easiest way to install a stable release through CRAN. Alternatively, you can acquire a development version from Github.
...
### Useful downloads
- Crosstable of functions and arguments [PDF](https://sommergeo.com/roadDB/functions_and_arguments.pdf)
- Tutorial script [PDF](https://sommergeo.com/roadDB/tutorial-script.pdf)
- Functions used in the tutorial [PDF](https://sommergeo.com/roadDB/tutorial-functions.pdf)
...
However, due to its complexity and expert-level focus, the database can be challenging to navigate and analyze directly. This is why the `roadDB` package was implemented as a short-cut into the R ecosystem.
...
The roadDB functions are essentially wrappers around complex SQL queries. They interact with a remote copy of the official ROAD database, which is updated four times per year. The current database version number can be found on the [Github page](https://github.com/sommergeo/roadDB?tab=readme-ov-file#cloud-database-status).
...
* LOD 1: Locality level
    - `road_get_localities()`
* LOD 2: Assemblage level
...
Once you have transferred your dataset to a spatial data type, there are many interesting packages available, that make use of the spatial data in ROAD:
...
If we stay with our Mousterian people for a bit, we can go into more detail and look at the lithic tools that are present at the localities using the `road_get_lithictypology()` function. But since the Mousterian concept exists in many different classifications, we need to now the exact technocomplexes to query. Using the `road_list_argument_values()` function.
...
This tool list as a bit hard to get an overview of, but if we know what we´re  looking for, then we can specify this using the `tool_list()` function which can be used together with the `road_get_lithic_typology()` function. We can first use the `road_list_argument_values()` function to see what our choices are.

```{r eval=FALSE}
road_list_argument_values("tool_list")
```
```{r echo=FALSE}
cat(paste0(paste(head(road_list_argument_values("tool_list")$tool_list,40), collapse = ", "), ", and many more."))
```
...
```{r}
mous_levallois <- road_get_lithic_typology(technocomplexes = df_mousterian,
                                           tool_list = "levallois")
```
...
Ostrich egg shells are an interesting material with a variety of uses. In this exercise, we'll create a quick graph to visualize their appearance across different cultural periods.
...
We then combine the two data frames using the [`merge`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/merge) function.

```{r}
# Query from miscellaneous finds
eggshell_misc <- road_get_miscellaneous_finds() # download whole dataframe
eggshell_misc <- eggshell_misc[grepl("ostrich egg shell", eggshell_misc$miscellaneous_finds_material), ] # filter rows for search pattern

# Query from symbolic artifacts
eggshell_symbolic <- road_get_symbolic_artifacts() # download whole dataframe
eggshell_symbolic <- eggshell_symbolic[grepl("ostrich egg shell", eggshell_symbolic$symbolic_artifacts_material), ] # filter rows for search pattern
...
First, we'll get an overview of all the available data, and then use `grepl()` to narrow down our results to rows, where the column `locality_id` contains the value "Hohle Fels".
...
We can extend our filter to select this specific archaeological layer and symbolic artifats.
...
```{r message=FALSE, warning=FALSE}
# Visualize all dates, with numbers indicating row number
