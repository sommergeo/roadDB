...
The tutorial uses the [pipe](https://tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/) operator `|>`, which is a native function since R v4.1, allowing to forward the results from one function into the next one (e.g. `function1() |> function2()`). You can write it by pressing `STRG+SHIFT+M`.
...
In this example, we visualize different technocomplex categories by setting the `col` argument (which determines which variable is used for coloring — here, `mousterian$technocomplex`) and the palette argument (which specifies the color scheme to use).
...
We can calibrate the layer Vb ages using the R package [`rcarbon`](https://cran.r-project.org/web/packages/rcarbon/vignettes/rcarbon.html). Since the site is located in the Northern Hemisphere, we'll use the "IntCal20" calibration curve ([Reimer et al. 2020](https://doi.org/10.1017/RDC.2020.41)).
